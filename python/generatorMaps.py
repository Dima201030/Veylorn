...
# Thank you, https://github.com/DanaL
# Github project -- https://github.com/DanaL/RLDungeonGenerator
...
class RLDungeonGenerator:
    ...
    def carve_corridor_between_rooms(self, room1, room2):
        if room2[2] == 'rows':
            row = choice(room2[1])
            start_col = min(room1.col + room1.width, room2[0].col + room2[0].width)
            end_col = max(room1.col, room2[0].col)
            ...
            col = choice(room2[1])
            start_row = min(room1.row + room1.height, room2[0].row + room2[0].height)
            end_row = max(room1.row, room2[0].row)
    ...
    def has_space_in_every_row(self) -> bool:
        for r in range(self.height):
            for c in range(self.width):
                if self.dungeon[r][c].get_ch() in (' ', '+'):
                    break
            else:
                return False
        return True
    ...
    def place_keys(self):
        for room in self.rooms:
            ...
            r, c = choice(possible_cells)
            self.dungeon[r][c] = DungeonSqr('g')
    ...
    def generate_map(self):
        ...
        for _ in range(MAX_ATTEMPTS):
            ...
            self.carve_rooms()
            self.connect_rooms()
            self.place_keys()  # ← ВАЖНО

            if self.has_space_in_every_row():
                return
    ...
    def find_spawn_point(self):
        for r in range(self.height):
            for c in range(self.width):
                if self.dungeon[r][c].get_ch() == ' ':
                    return r, c
        raise RuntimeError("No empty space for player spawn")

    def place_player(self, symbol='@'):
        r, c = self.find_spawn_point()
        self.dungeon[r][c] = DungeonSqr(symbol)
        return r, c

    def save_to_file(self, filepath: str):
        spawn_r, spawn_c = self.place_player('@')

        with open(filepath, "w", encoding="utf-8") as f:
            for r in range(self.height):
                row = ''.join(self.dungeon[r][c].get_ch() for c in range(self.width))
                f.write(row + "\n")

            f.write(f"fpx={spawn_c},fpy={spawn_r},s=@\n")

def generate_map_to_file(path: str, width=75, height=40):
    dg = RLDungeonGenerator(width, height)
    dg.generate_map()      # здесь уже создаются комнаты, коридоры и КЛЮЧИ
    dg.save_to_file(path)
